...
const transformValue = value => {
	if (value.__type === 'Pointer') {
		return value[value.__class_prefix+'codigo'];
	}
	return value;
}
	...
	if (schema) {
		Object.keys(schema.fields).forEach((field) => {
			if (schema.fields[field].type === 'Relation') {
				list.push(`__join:${field}:${schema.className}`);
			}
		});
	}
	return list;
...
export class PostgresStorageAdapter {
	...
	_ensureSchemaCollectionExists(conn) {
		...
		return conn.none('CREATE TABLE IF NOT EXISTS "db_schema" ( "sch_class_name" varChar(120), "sch_schema" jsonb, "sch_is_parse_class" bool, PRIMARY KEY ("sch_class_name") )')
			.catch(error => {
	...
	setClassLevelPermissions(className, CLPs) {
		return this._ensureSchemaCollectionExists().then(() => {
			const values = [className, 'sch_schema', 'classLevelPermissions', JSON.stringify(CLPs)]
			return this._client.none(`UPDATE "db_schema" SET $2:name = json_object_set_key($2:name, $3::text, $4::jsonb) WHERE "sch_class_name"=$1 `, values);
		});
	}

	createClass(className, schema) {
		console.log(className, schema);
		return this._client.tx(t => {
			const q1 = this.createTable(className, schema, t);
			const q2 = t.none('INSERT INTO "db_schema" ("sch_class_name", "sch_schema", "sch_is_parse_class") VALUES ($<className>, $<schema>, true)', { className, schema });
	...
	createTable(className, schema, conn) {
		...
		Object.keys(fields).forEach((fieldName) => {
			...
			if (fieldName === `${schema.__class_prefix}codigo`) {
		...
		return this._ensureSchemaCollectionExists(conn)
			...
			.catch(error => {
				...
				return Promise.all(relations.map((fieldName) => {
					return conn.none('CREATE TABLE IF NOT EXISTS $<joinTable:name> ("relacionado_codigo" varChar(120), "dono_codigo" varChar(120), PRIMARY KEY("relacionado_codigo", "dono_codigo") )', { joinTable: `__join:${fieldName}:${className}` });
				}));
			});
	}
	...
	addFieldIfNotExists(className, fieldName, type) {
		...
		return this._client.tx("addFieldIfNotExists", t => {
			...
			if (type.type !== 'Relation') {
				...
				promise = t.none('CREATE TABLE IF NOT EXISTS $<joinTable:name> ("relacionado_codigo" varChar(120), "dono_codigo" varChar(120), PRIMARY KEY("relacionado_codigo", "dono_codigo") )', { joinTable: `__join:${fieldName}:${className}` })
			}
			return promise.then(() => {
				return t.any('SELECT "sch_schema" FROM "db_schema" WHERE "sch_class_name" = $<className> and ("sch_schema"::json->\'fields\'->$<fieldName>) is not null', { className, fieldName });
			}).then(result => {
				if (result[0]) {
					...
					return t.none(
						'UPDATE "db_schema" SET "sch_schema"=jsonb_set("sch_schema", $<path>, $<type>)  WHERE "sch_class_name"=$<className>',
						{ path, type, className }
					);
				}
			});
	...
	deleteClass(className) {
		return Promise.resolve().then(() => {
			...
			[`DELETE FROM "db_schema" WHERE "sch_class_name"=$1`, [className]]];
			...
			return className.indexOf('__join:') != 0;
		});
	}
	...
	deleteAllClasses() {
		...
		return this._client.any('SELECT * FROM "db_schema"')
			.then(results => {
				const joins = results.reduce((list, schema) => {
					return list.concat(joinTablesForSchema(schema.schema));
				}, []);
				const classes = ['db_schema', 'db_push_status', 'db_job_status', 'db_hooks', 'db_global_config', ...results.map(result => result.className), ...joins];
				return this._client.tx(t => t.batch(classes.map(className => t.none('DROP TABLE IF EXISTS $<className:name>', { className }))));
			}, error => {
				if (error.code === PostgresRelationDoesNotExistError) {
					// No db_schema collection. Don't delete anything.
					return;
				} else {
					throw error;
				}
	...
	// the db_schema collection.  There should be no actual data in the collection under the same name
	...
	deleteFields(className, schema, fieldNames) {
		...
		return Promise.resolve()
			.then(() => {
				...
				const doBatch = (t) => {
					const batch = [
						t.none('UPDATE "db_schema" SET "sch_schema"=$<schema> WHERE "sch_class_name"=$<className>', { schema, className })
					];
					if (values.length > 1) {
	...
	getAllClasses() {
		return this._ensureSchemaCollectionExists()
			.then(() => this._client.map('SELECT * FROM "db_schema"', null, row => ({ sch_class_name: row.className, ...row.schema })))
	...
	getClass(className) {
		...
		return this._client.any('SELECT * FROM "db_schema" WHERE "sch_class_name"=$<className>', { className })
			.then(result => {
				if (result.length === 1) {
	...
	createObject(className, schema, object) {
		...
		Object.keys(object).forEach(fieldName => {
			...
			if (!schema.fields[fieldName] && className === UserClass) {
